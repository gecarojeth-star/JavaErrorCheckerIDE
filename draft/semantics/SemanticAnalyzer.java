...
public class SemanticAnalyzer {
    ...
    private void checkDuplicates(List<Token> tokens) {
        ...
        checkNullAssignments(tokens);
    }

    private void parseDeclarations(List<Token> tokens) {
        for (int i = 0; i < tokens.size(); i++) {
            Token token = tokens.get(i);
            if (token instanceof SymbolToken) {
                if (token.lexeme.equals("{")) symbolTable.enterScope();
                if (token.lexeme.equals("}")) symbolTable.exitScope();
                continue;
            }

            if (!isTypeToken(token)) continue;
            String type = typeNameFromToken(token);
            int current = i + 1;

            while (current + 1 < tokens.size() && tokens.get(current) instanceof SymbolToken && tokens.get(current).lexeme.equals("[") &&
                   tokens.get(current + 1) instanceof SymbolToken && tokens.get(current + 1).lexeme.equals("]")) {
                type += "[]";
                current += 2;
            }

            if (current >= tokens.size() || !(tokens.get(current) instanceof IdentifierToken)) continue;
            Token nameToken = tokens.get(current);
            int next = current + 1;

            if (next < tokens.size() && tokens.get(next) instanceof SymbolToken && tokens.get(next).lexeme.equals("(")) {
                if (!(i > 0 && tokens.get(i - 1) instanceof KeywordToken && tokens.get(i - 1).lexeme.equals("new"))) {
                    // Skip method signature / constructor call
                }
                i = current;
                continue;
            }
            ...
            while (true) {
                if (nameToken instanceof IdentifierToken) {
                    try {
                        symbolTable.declare(nameToken.lexeme, type);
                    } catch (Exception e) {
                        errors.add(new ErrorDetail(ErrorDetail.Severity.ERROR, "Semantic",
                                "Duplicate variable '" + nameToken.lexeme + "'",
                                "Rename or reuse existing variable",
                                nameToken.line, nameToken.col, nameToken.col + nameToken.lexeme.length()));
                    }
                }

                int assignmentIndex = next;
                if (assignmentIndex < tokens.size() && tokens.get(assignmentIndex) instanceof OperatorToken && tokens.get(assignmentIndex).lexeme.equals("=")) {
                    int valueIndex = assignmentIndex + 1;
                    if (valueIndex < tokens.size() && tokenIsNullLiteral(tokens.get(valueIndex)) && isPrimitiveType(type)) {
                        errors.add(new ErrorDetail(ErrorDetail.Severity.ERROR, "Semantic",
                                "Cannot assign null to primitive type '" + type + "'",
                                "Use a reference type or a default primitive value",
                                tokens.get(valueIndex).line, tokens.get(valueIndex).col, tokens.get(valueIndex).col + tokens.get(valueIndex).lexeme.length()));
                    }
                }

                if (next < tokens.size() && tokens.get(next) instanceof SymbolToken && tokens.get(next).lexeme.equals(",")) {
                    next++;
                    if (next < tokens.size() && tokens.get(next) instanceof IdentifierToken) {
                        nameToken = tokens.get(next);
                        next++;
                        continue;
                    }
                }
    ...
    private void checkNullAssignments(List<Token> tokens) {
        for (int i = 0; i + 2 < tokens.size(); i++) {
            if (!(tokens.get(i) instanceof IdentifierToken)) continue;
            Token assign = tokens.get(i + 1);
            if (!(assign instanceof OperatorToken) || !assign.lexeme.equals("=")) continue;
            Token value = tokens.get(i + 2);
            if (!tokenIsNullLiteral(value)) continue;
            String variableType = symbolTable.lookup(tokens.get(i).lexeme);
            if (variableType != null && isPrimitiveType(variableType)) {
                errors.add(new ErrorDetail(ErrorDetail.Severity.ERROR, "Semantic",
                        "Cannot assign null to primitive type '" + variableType + "'",
                        "Use a reference type or a default primitive value",
                        value.line, value.col, value.col + value.lexeme.length()));
            }
        }
    }
    ...
    private boolean isTypeToken(Token token) {
        if (token instanceof KeywordToken) {
            ...
            return PRIMITIVE_TYPES.contains(lexeme) || lexeme.equals("String");
        }
        if (token instanceof IdentifierToken) {
            return Character.isUpperCase(token.lexeme.charAt(0));
        }
        return false;
    }

    private String typeNameFromToken(Token token) {
    ...
    private boolean tokenIsNullLiteral(Token token) {
        return token instanceof KeywordToken && token.lexeme.equals("null");
    }

    private boolean isPrimitiveType(String type) {
        return PRIMITIVE_TYPES.contains(type.replace("[]", ""));
    }
    ...
    private void checkUnreachable(String cleanSrc) {
        String[] lines = cleanSrc.split("\n");
        Deque<Boolean> deadStack = new ArrayDeque<>();
        deadStack.push(false);
        int deadLine = 0;
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            int openCount = countChar(ln, '{');
            int closeCount = countChar(ln, '}');
            for (int j = 0; j < openCount; j++) {
                deadStack.push(deadStack.peek());
            }

            if (deadStack.peek() && !ln.equals("{") && !ln.equals("}")) {
                int[] offs = getOffsetForLine(cleanSrc, i + 1);
                errors.add(new ErrorDetail(ErrorDetail.Severity.WARNING, "Semantic",
            ...
            if (ln.matches("^\\s*(return|break|continue|throw)\\b.*;\\s*$")) {
                if (!deadStack.isEmpty()) {
                    deadLine = i + 1;
                    deadStack.pop();
                    deadStack.push(true);
            ...
            for (int j = 0; j < closeCount; j++) {
                if (deadStack.size() > 1) deadStack.pop();
            }
        }
    }
